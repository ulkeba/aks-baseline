...
1. Create the networking hubs resource group.
   ...
   az group create -n ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-hubs -l ${TARGET_REGION_AKS_BASELINE}
   ```
...
1. Create the networking spokes resource group.
   ...
   ```bash
   # [This takes less than one minute to run.]
   az group create -n ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-spokes -l ${TARGET_REGION_AKS_BASELINE}
   ```
...
1. Create the regional network hub.
   ...
   az deployment group create -g ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-hubs -f networking/hub-default.bicep -p prefix=${PREFIX_AKS_BASELINE} -p location=${TARGET_REGION_AKS_BASELINE}
   ```

   The hub creation will emit the following:

      * `hubVnetId` - which you'll will query in future steps when creating connected regional spokes. E.g. `/subscriptions/[id]/resourceGroups/[your prefix]-enterprise-networking-hubs/providers/Microsoft.Network/virtualNetworks/vnet-eastus2-hub`
...
1. Create the spoke that will be home to the AKS cluster and its adjacent resources.
   ...
   ```bash
   RESOURCEID_VNET_HUB=$(az deployment group show -g ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-hubs -n hub-default --query properties.outputs.hubVnetId.value -o tsv)
   echo RESOURCEID_VNET_HUB: $RESOURCEID_VNET_HUB

   # [This takes about four minutes to run.]
   az deployment group create -g ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-spokes -f networking/spoke-BU0001A0008.bicep -p prefix=${PREFIX_AKS_BASELINE} -p location=${TARGET_REGION_AKS_BASELINE} -p hubVnetResourceId="${RESOURCEID_VNET_HUB}"
   ```
