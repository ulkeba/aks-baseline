...
1. Create the AKS cluster resource group.
   ...
   az group create --name ${PREFIX_AKS_BASELINE}-rg-bu0001a0008 --location ${TARGET_REGION_AKS_BASELINE}
   ```
...
1. Get the AKS cluster spoke virtual network resource ID.
   ...
   ```bash
   export RESOURCEID_VNET_CLUSTERSPOKE_AKS_BASELINE=$(az deployment group show -g ${PREFIX_AKS_BASELINE}-rg-enterprise-networking-spokes -n spoke-BU0001A0008 --query properties.outputs.clusterVnetResourceId.value -o tsv)
...
1. Deploy the container registry template.
   ...
   az deployment group create -g ${PREFIX_AKS_BASELINE}-rg-bu0001a0008 -f acr-stamp.bicep -p prefix=${PREFIX_AKS_BASELINE} -p location=${TARGET_REGION_AKS_BASELINE} -p targetVnetResourceId=${RESOURCEID_VNET_CLUSTERSPOKE_AKS_BASELINE}
   ```
...
1. Import cluster management images to your container registry.
   ...
   export ACR_NAME_AKS_BASELINE=$(az deployment group show -g ${PREFIX_AKS_BASELINE}-rg-bu0001a0008 -n acr-stamp --query properties.outputs.containerRegistryName.value -o tsv)
